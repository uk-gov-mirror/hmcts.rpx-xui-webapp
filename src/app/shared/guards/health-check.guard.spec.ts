...
describe('HealthCheckGuard', () => {
  ...
  describe('canActivate', () => {
    it('should trigger service call', inject(
      [HealthCheckGuard],
      (guard: HealthCheckGuard) => {
    ...
    it('should trigger service down when there is a false healthstate', inject(
      [HealthCheckGuard],
      (guard: HealthCheckGuard) => {
    ...
    it('should trigger service down when there is an error', inject(
      [HealthCheckGuard],
      (guard: HealthCheckGuard) => {
        healthCheckServiceMock.doHealthCheck.and.returnValue(
          Observable.throwError({ status: 404 })
        );
        guard.canActivate().subscribe(() => {
          expect(storeInstance.dispatch).toHaveBeenCalled();
        });
