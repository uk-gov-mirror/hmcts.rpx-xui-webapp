...
export class MediaViewerWrapperComponent implements OnInit {
  ...
  public async ngOnInit() {
    ...
    this.icpJurisdictions$ = this.featureToggleService.getValue(
      'icp-jurisdictions',
      []
    );
  ...
  public isIcpEnabled(isEnabled: boolean, icpJurisdictions: string[]): boolean {
    return icpJurisdictions && icpJurisdictions.length > 0
      ? icpJurisdictions.includes(this.caseJurisdiction)
      : isEnabled;
  }
}
