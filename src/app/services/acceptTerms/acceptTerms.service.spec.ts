...
describe('Accept terms And Conditions Service', () => {
  it('should be created And Truthy', () => {
    const mockHttpClient = jasmine.createSpyObj('mockHttpClient', [
      'get',
      'post',
    ]);
  ...
  it('should postUserAccepted', () => {
    ...
    expect(
      mockHttpClient.post
    ).toHaveBeenCalledWith(`api/userTermsAndConditions`, { userId: 'userId' });
  });
  ...
  it('should getIsUserAccepted', () => {
    ...
    expect(mockHttpClient.get).toHaveBeenCalledWith(
      `api/userTermsAndConditions/userId`
    );
  });
