...
describe('Logger service', () => {
  const mockedMonitoringService = jasmine.createSpyObj(
    'mockedMonitoringService',
    ['logEvent', 'logException']
  );
  const mockedNgxLogger = jasmine.createSpyObj('mockedNgxLogger', [
    'trace',
    'debug',
    'info',
    'log',
  ...
  const mockedSessionStorageService = jasmine.createSpyObj(
    'mockedCookieService',
    ['getItem']
  );
  const mockedCryptoWrapper = jasmine.createSpyObj('mockedCryptoWrapper', [
    'encrypt',
    'decrypt',
  ]);
  ...
  it('should be able to get a message', () => {
    ...
    const expectedMessage = `Message - message, Timestamp - ${Date.now()}`.slice(
      0,
      -2
    );
