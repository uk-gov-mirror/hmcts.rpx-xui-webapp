...
export class FooterComponent implements OnInit {
  ...
  public ngOnInit() {
    const tAndCNavItem = this.getNavigationItemForTandC(
      this.navigationData.items
    );
    if (tAndCNavItem) {
      this.store
        .pipe(select(fromRoot.getIsTermsAndConditionsFeatureEnabled))
        .subscribe((isEnabled) => {
          tAndCNavItem.href = isEnabled
  ...
  public getNavigationItemForTandC(
