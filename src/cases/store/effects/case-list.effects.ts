...
export class CaseListEffects {
  ...
  public applyPageMetadata$ = this.actions$.pipe(
    ...
    map(
      (action: caselistActions.FindCaselistPaginationMetadata) => action.payload
    ),
    switchMap((payload) => {
      ...
      return this.searchService.findPaginationMetadata(payload).pipe(
        map(
          (response) =>
            new caselistActions.FindCaselistPaginationMetadataSuccess(
              response.json()
            )
        ),
        catchError((error) =>
          of(new caselistActions.ApplyCaselistFilterFail(error))
        )
      );
    })
  );
      ...
      return this.searchService.search(payload).pipe(
        map(
          (result: Observable<any>) =>
            new caselistActions.ApplyCaselistFilterSuccess(result)
        ),
        catchError((error) =>
          of(new caselistActions.ApplyCaselistFilterFail(error))
        )
  ...
  public applySearchFilterToggle$ = this.actions$.pipe(
    ...
    map(
      (action: caselistActions.CaseFilterToggle) =>
        new caselistActions.CaseFilterToggleSuccess(action.payload)
    ),
    catchError((error) =>
      of(new caselistActions.ApplyCaselistFilterFail(error))
    )
