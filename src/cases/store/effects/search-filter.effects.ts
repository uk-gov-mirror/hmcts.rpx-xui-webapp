...
export class SearchFilterEffects {
  ...
  public applyPageMetadata$ = this.actions$.pipe(
    ...
    map(
      (action: caseSearchActions.FindSearchPaginationMetadata) => action.payload
    ),
    switchMap((payload) => {
      ...
      return this.searchService.findPaginationMetadata(payload).pipe(
        map(
          (response) =>
            new caseSearchActions.FindSearchPaginationMetadataSuccess(
              response.json()
            )
        ),
        catchError((error) =>
          of(new caseSearchActions.ApplySearchFilterFail(error))
        )
      );
    })
  );
      ...
      return this.searchService.search(payload).pipe(
        map(
          (result: Observable<any>) =>
            new caseSearchActions.ApplySearchFilterSuccess(result)
        ),
        catchError((error) =>
          of(new caseSearchActions.ApplySearchFilterFail(error))
        )
  ...
  public applySearchFilterToggle$ = this.actions$.pipe(
    ...
    map(
      (action: caseSearchActions.SearchFilterToggle) =>
        new caseSearchActions.SearchFilterToggleSuccess(action.payload)
    ),
    catchError((error) =>
      of(new caseSearchActions.ApplySearchFilterFail(error))
    )
