...
describe('SearchFilterService', () => {
  ...
  const ccdSearchServiceMock = createSpyObj<SearchService>('SearchService', [
    'search',
    'searchCases',
  ]);
  const abstractAppConfigMock = createSpyObj<AbstractAppConfig>(
    'AbstractAppConfig',
    ['getCaseDataUrl']
  );
  const httpService = createSpyObj<HttpService>('HttpService', ['get']);
  const requestOptionsBuilder = createSpyObj<RequestOptionsBuilder>(
    'RequestOptionsBuilder',
    ['buildOptions']
  );
