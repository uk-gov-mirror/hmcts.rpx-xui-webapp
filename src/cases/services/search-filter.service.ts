...
export class SearchFilterService {
  ...
  public search(
    payload,
    isElasticSearchEnabled: boolean = false
  ): Observable<any> {
    const {
      jurisdictionId,
      caseTypeId,
    ...
    return isElasticSearchEnabled
      ? (this.ccdSearchService.searchCases(
          caseTypeId,
          metadataFilters,
          caseFilters,
          view,
  ...
  private getCaseFilterFromFormGroup(
    formGroup?: FormGroup
  ): { caseFilter; metadataFilter } {
    const result = {
      caseFilter: {},
      metadataFilter: {},
    };
  ...
  public findPaginationMetadata(payload): Observable<any> {
    ...
    const options = this.requestOptionsBuilder.buildOptions(
      metadataFilters,
      caseFilters
    );
