...
describe('CaseListComponent', () => {
  ...
  const mockDefinitionsService = jasmine.createSpyObj('DefinitionsService', [
    'getJurisdictions',
  ]);
  const mockAppConfig = jasmine.createSpyObj('AppConfig', [
    'getPaginationPageSize',
  ]);
  const mockWindowService = jasmine.createSpyObj('WindowService', [
    'removeLocalStorage',
  ]);
  ...
  describe('getToggleButtonName()', () => {
    it(
      "should return the toggle button name as 'Hide Filter' if we have shown " +
        'the filter',
      () => {
    ...
    it(
      "should return the toggle button name as 'Show Filter' if we do not show " +
        'the filter',
      () => {
  ...
  describe('findCaseListPaginationMetadata()', () => {
    ...
    it('should dispatch an action to find the case list pagination metadata.', () => {
      ...
      expect(spyOnDispatchToStore).toHaveBeenCalledWith(
        new FindCaselistPaginationMetadata(event)
      );
    });
  });
  ...
  describe('getElasticSearchResults', () => {
    it('should dispatch an action to get results from elastic search endpoint.', () => {
      ...
      expect(spyOnDispatchToStore).toHaveBeenCalledWith(
        new ApplyCaselistFilterForES(event)
      );
    });
  });
  ...
  describe('applyChangePage()', () => {
    ...
    it('should call findCaseListPaginationMetadata() on page change.', () => {
      const spyOnFindCaseListPaginationMetadata = spyOn(
        component,
        'findCaseListPaginationMetadata'
      ).and.callThrough();
    ...
    it('should call getElasticSearchResults() on page change and LD elastic search enabled.', () => {
      const spyOnGetElasticSearchResults = spyOn(
        component,
  ...
  describe('onPaginationSubscribeHandler()', () => {
    it('should update the components paginationMetadata property, on return of subscription.', () => {
      ...
      expect(component.paginationMetadata.total_pages_count).toEqual(
        paginationMetadata.total_pages_count
      );
      expect(component.paginationMetadata.total_results_count).toEqual(
  ...
  describe('Should show share case button', () => {
    ...
    it("Should see the 'Share case' button greyed out", () => {
      ...
      spyOnProperty(component, 'isCaseShareVisible$').and.returnValue(
        Observable.of(true)
      );
      fixture.detectChanges();
      expect(
        fixture.debugElement.nativeElement.querySelector('#btn-share-button')
          .textContent
      ).toContain('Share Case');
    ...
    it('Should save share cases to store when selection is changed', () => {
      component.retrieveSelections(selectedCases);
      expect(spyOnDispatchToStore).toHaveBeenCalledWith(
        new SynchronizeStateToStore(
          converts.toShareCaseConverter(component.selectedCases)
        )
      );
    });
  ...
  describe('Should see cases unselectable information', () => {
    ...
    it('should see why are some cases unselectable', () => {
      ...
      fixture.detectChanges();
      const infoHeader = fixture.debugElement.query(
        By.css('#sp-msg-unselected-case-header')
      ).nativeElement;
      expect(infoHeader.innerHTML).toContain(
        'Why are some cases unselectable?'
      );
      const infoContent = fixture.debugElement.query(
        By.css('#sp-msg-unselected-case-content')
      ).nativeElement;
  ...
  describe('onDestroy()', () => {
    it('should unsubscribe', () => {
      ...
      spyOn(
        component.caseFilterToggleSubscription,
        'unsubscribe'
      ).and.callThrough();
      ...
      expect(
        component.caseFilterToggleSubscription.unsubscribe
      ).toHaveBeenCalled();
    });
  });
