...
export class CaseListComponent implements OnInit, OnDestroy {
  ...
  public jurisdictionsBehaviourSubject$: BehaviorSubject<
    Jurisdiction[]
  > = new BehaviorSubject<Jurisdiction[]>([]);
  ...
  public ngOnInit() {
    ...
    this.definitionsService
      .getJurisdictions('read')
      .subscribe(this.jurisdictionsBehaviourSubject$);
    ...
    this.shareableJurisdictions$ = this.featureToggleService.getValue(
      'shareable-jurisdictions',
      []
    );
    ...
    this.jurisdiction$ = this.store.pipe(
      select(fromCasesFeature.caselistFilterJurisdiction)
    );
    this.caseType$ = this.store.pipe(
      select(fromCasesFeature.caselistFilterCaseType)
    );
    this.caseState$ = this.store.pipe(
      select(fromCasesFeature.caselistFilterCaseState)
    );
    this.metadataFields$ = this.store.pipe(
      select(fromCasesFeature.caselistFilterMetadataFields)
    );
    ...
    this.caseFilterToggle$ = this.store.pipe(
      select(fromCasesFeature.getCaselistFilterToggle)
    );
    this.caseFilterToggleSubscription = this.caseFilterToggle$.subscribe(
      (result: boolean) => this.onToogleHandler(result)
    );

    this.listenToPaginationMetadata();

    this.resultView$ = this.store.pipe(
      select(fromCasesFeature.caselistFilterResultView)
    );
    ...
    this.elasticSearchFlagSubsription = this.featureToggleService
      .isEnabled('elastic-search')
      .subscribe((value) => (this.elasticSearchFlag = value));
    ...
    this.shareCases$.subscribe(
      (shareCases) =>
        (this.selectedCases = converters.toSearchResultViewItemConverter(
          shareCases
        ))
    );
  }
  ...
  public listenToPaginationMetadata = () => {
    ...
    this.paginationSubscription = this.paginationMetadata$.subscribe(
      (paginationMetadata) =>
  ...
  public setCaseListFilterDefaults = () => {
    this.jurisdictionsBehaviourSubject$.subscribe((jurisdictions) => {
      if (jurisdictions.length > 0) {
        this.savedQueryParams = JSON.parse(
    ...
    if (this.elasticSearchFlag) {
      const paginationDataFromResult: PaginationMetadata = {
        ...
        total_pages_count: Math.ceil(
          resultView.total / this.appConfig.getPaginationPageSize()
        ),
      };
  ...
  public onPaginationSubscribeHandler = (paginationMetadata) => {
    if (
      typeof paginationMetadata !== 'undefined' &&
      typeof paginationMetadata.total_pages_count !== 'undefined'
    ) {
      this.paginationMetadata.total_pages_count =
        paginationMetadata.total_pages_count;
      this.paginationMetadata.total_results_count =
        paginationMetadata.total_results_count;
  ...
  public getEvent() {
    ...
    if (this.selected) {
      ...
      formGroupFromLS = JSON.parse(
        localStorage.getItem('workbasket-filter-form-group-value')
      );
  ...
  public findCaseListPaginationMetadata(event) {
    if (event !== null) {
      this.store.dispatch(
        new fromCasesFeature.FindCaselistPaginationMetadata(event)
      );
    }
  }
  ...
  public applyFilter(event) {
    ...
    if (this.ccdSearchResult) {
      ...
      this.ccdSearchResult.consumerSortParameters = {
        column: null,
        order: null,
        type: null,
      };
  ...
  public toggleFilter() {
    this.store.dispatch(
      new fromCasesFeature.CaseFilterToggle(!this.showFilter)
    );
  }
  ...
  public caseShareIsVisible(project: any[]): boolean {
    ...
    if (userDetails && shareableJurisdictions && jurisdiction) {
      return (
        userDetails.canShareCases &&
        shareableJurisdictions.includes(jurisdiction.id)
      );
    }
    return false;
  }
  ...
  public retrieveSelections(event) {
    ...
    this.store.dispatch(
      new fromCasesFeature.SynchronizeStateToStore(
