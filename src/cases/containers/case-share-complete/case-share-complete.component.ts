...
export class CaseShareCompleteComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit() {
    ...
    this.store.dispatch(
      new fromCasesFeature.AssignUsersToCase(this.shareCases)
    );

    this.shareCaseState$ = this.store.pipe(
      select(fromCasesFeature.getCaseShareState)
    );
    this.shareCaseState$.subscribe((state) => (this.isLoading = state.loading));
    this.newShareCases$ = this.store.pipe(
      select(fromCasesFeature.getShareCaseListState)
    );
  ...
  public checkIfIncomplete(shareCases: SharedCase[]) {
    if (this.isLoading) {
      if (
        shareCases.some(
          (aCase) => aCase.pendingShares && aCase.pendingShares.length > 0
        )
      ) {
        return 'PENDING';
      }
      if (
        shareCases.some(
          (aCase) => aCase.pendingUnshares && aCase.pendingUnshares.length > 0
        )
      ) {
        return 'PENDING';
      }
