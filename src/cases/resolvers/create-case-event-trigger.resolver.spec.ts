...
describe('CreateCaseFieldsResolver', () => {
  const PARAM_JURISDICTION_ID =
    CreateCaseEventTriggerResolver.PARAM_JURISDICTION_ID;
  ...
  const QUERY_PARAM_IGNORE_WARNINGS =
  ...
  const EVENT_TRIGGER_OBS: Observable<CaseEventTrigger> = Observable.of(
    EVENT_TRIGGER
  );
  ...
  it('should resolve event trigger and cache when route is :jid/:ctid/:eid', () => {
    ...
    expect(route.queryParamMap.get).toHaveBeenCalledWith(
      QUERY_PARAM_IGNORE_WARNINGS
    );
