...
export class CreateCaseEventTriggerResolver
  implements Resolve<CaseEventTrigger> {
  ...
  public resolve(route: ActivatedRouteSnapshot): Observable<CaseEventTrigger> {
    return this.isRootCreateRoute(route)
      ...
      ? Observable.of(this.cachedEventTrigger)
      : this.getAndCacheEventTrigger(route);
  }

  public getAndCacheEventTrigger(
    route: ActivatedRouteSnapshot
  ): Observable<CaseEventTrigger> {
    const caseTypeId = route.paramMap.get(
      CreateCaseEventTriggerResolver.PARAM_CASE_TYPE_ID
    );
    const eventTriggerId = route.paramMap.get(
      CreateCaseEventTriggerResolver.PARAM_EVENT_ID
    );
    let ignoreWarning = route.queryParamMap.get(
      CreateCaseEventTriggerResolver.QUERY_PARAM_IGNORE_WARNING
    );
    ...
    if (
      -1 ===
      CreateCaseEventTriggerResolver.IGNORE_WARNING_VALUES.indexOf(
        ignoreWarning
      )
    ) {
      ignoreWarning = 'false';
    ...
    return this.casesService.getEventTrigger(
      caseTypeId,
      eventTriggerId,
      caseId,
      ignoreWarning
