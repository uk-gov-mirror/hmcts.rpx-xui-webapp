...
export function toShareCaseConverter(
  ...
  for (const selectCase of selectedCases) {
    ...
    const shareCase = {
      ...
      caseTitle: caseTitle
        ? caseTitle
        : combineCaseTitleByCaseType(caseTypeId, selectCase),
      caseTypeId,
    };
...
export function toSearchResultViewItemConverter(
...
function getValueByPropertyName(
  selectCase: SearchResultViewItem,
  propName: string
): any {
  return selectCase.case_fields &&
    selectCase.case_fields.hasOwnProperty(propName)
    ? selectCase.case_fields[propName]
    : '';
}

function combineCaseTitleByCaseType(
  caseTypeId: string,
  selectCase: SearchResultViewItem
): string {
  if (caseTypeId.includes('FinancialRemedy')) {
    ...
    return (
      applicantName + showVersus(applicantName, respondentName) + respondentName
    );
    ...
    return (
      ...
      showVersus(marriagePetitionerName, marriageRespondentName) +
...
function getApplicantName(selectCase: SearchResultViewItem) {
  return getValueByPropertyName(selectCase, 'applicantFMName') &&
    getValueByPropertyName(selectCase, 'applicantLName')
    ? getValueByPropertyName(selectCase, 'applicantFMName') +
        BLANK_SPACE +
        getValueByPropertyName(selectCase, 'applicantLName')
    : getValueByPropertyName(selectCase, 'applicantLName')
    ? getValueByPropertyName(selectCase, 'applicantLName')
