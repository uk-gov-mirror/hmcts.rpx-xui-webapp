...
describe('Noc Effects', () => {
  ...
  describe('setCaseReference$', () => {
    it('should return a response', () => {
      ...
      const completion = new nocActions.SetQuestions({
        questions: dummy,
        caseReference: '1223221244223131',
      });
    ...
    it('should redirect to service down', () => {
      ...
      nocServiceMock.getNoCQuestions.and.returnValue(
        throwError({ dummyError, status: 404 })
      );
      const action = new nocActions.SetCaseReference('1223-2212-4422-3131');
  ...
  describe('handleError', () => {
    it('should handle 400', () => {
      const action$ = NocEffects.handleError(
        { status: 400, message: 'error' },
        nocActions.SET_CASE_REFERENCE
      );
      action$.subscribe((action) =>
        expect(action).toEqual(
          new nocActions.SetCaseRefSubmissionFailure({
            status: 400,
            message: 'error',
          })
        )
  ...
  describe('handleError', () => {
    it('should handle 500', () => {
      ...
      action$.subscribe((action) =>
        expect(action).toEqual(new Go({ path: ['/service-down'] }))
      );
    });
  });
});
