    ...
    if (nocQuestion.answer_field_type.type) {
      ...
      if (nocQuestion.answer_field_type.type === 'Text') {
        if (nocQuestion.answer_field_type.regular_expression) {
          validators.push(
            Validators.pattern(nocQuestion.answer_field_type.regular_expression)
          );
        } else {
          validators.push(
            Validators.pattern(FormValidatorsService.REGEX_WHITESPACES)
          );
        }
        if (nocQuestion.answer_field_type.min) {
          validators.push(
            Validators.minLength(nocQuestion.answer_field_type.min)
          );
        }
        if (nocQuestion.answer_field_type.max) {
          validators.push(
            Validators.maxLength(nocQuestion.answer_field_type.max)
          );
        }
