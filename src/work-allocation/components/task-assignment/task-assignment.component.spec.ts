...
@Component({
  template: `<exui-task-assignment
    [excludeCaseworkers]="excludeCaseworkers"
    [location]="location"
    (caseworkerChanged)="onCaseworkerChanged($event)"
  ></exui-task-assignment>`,
})
...
describe('WorkAllocation', () => {
  describe('TaskAssignmentComponent', () => {
    ...
    it('should have appropriate options shown in the location select', () => {
      ...
      expect(select.options[0].label).toEqual(
        component.ALL_LOCATIONS.locationName
      );
    ...
    it('should have appropriate default options shown in the caseworker select', () => {
      ...
      expect(select.options[2].textContent).toEqual(
        'John Smith - j.s@cw.gov.uk'
      );
      expect(select.options[3].textContent).toEqual(
        'Joseph Bloggs - j.b@cw.gov.uk'
      );
      expect(select.options[4].textContent).toEqual(
        'Noah Body - n.b@cw.gov.uk'
      );
    });
    ...
    it('should appropriately filter any excluded caseworkers', () => {
      ...
      expect(select.options[0].label).toEqual('Select name');
      expect(select.options[1].textContent).toEqual(
        'John Smith - j.s@cw.gov.uk'
      );
      expect(select.options[2].textContent).toEqual(
        'Joseph Bloggs - j.b@cw.gov.uk'
    ...
    it('should handle the location being changed and refresh list of caseworkers', () => {
      const locationsSelect: HTMLSelectElement = getSelect(
        '#task_assignment_location'
      );
      ...
      fixture.whenStable().then(() => {
        ...
        const caseworkersSelect: HTMLSelectElement = getSelect(
          '#task_assignment_caseworker'
        );
        ...
        expect(caseworkersSelect.options[1].textContent).toEqual(
          'Jane Doe - j.d@cw.gov.uk'
        );
        expect(caseworkersSelect.options[2].textContent).toEqual(
          'John Smith - j.s@cw.gov.uk'
        );
      });
    });
