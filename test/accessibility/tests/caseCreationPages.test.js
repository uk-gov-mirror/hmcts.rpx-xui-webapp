...
describe('Pa11y Accessibility tests', function () {
  ...
  it('Create Case Page', async function () {
    ...
    const actions = [];
    actions.push(
      ...PallyActions.waitForPageWithCssLocator('exui-ccd-connector')
    );
    await pa11ytest(this, actions, conf.baseUrl + 'cases/case-filter');
  });

  const divorceCreateCaseConfig = ccdApi.getSolicitorCreateCaseConfig(
    'DIVORCE',
    'DIVORCE'
  );
  divorceCreateCaseConfig.wizard_pages.forEach((page) => {
    if (!page.show_condition) {
      it('Divorce Create Case Page ' + page.id, async function () {
        ...
        const actions = [];
        actions.push(
          ...PallyActions.waitForPageWithCssLocator('ccd-case-edit-page h1')
        );
        await pa11ytest(
          this,
          actions,
          conf.baseUrl +
            'cases/case-create/DIVORCE/DIVORCE/solicitorCreate/' +
            page.id
        );
      });
    }
  ...
  const fr_ConsentedConfig = ccdApi.getSolicitorCreateCaseConfig(
    'FinancialRemedyMVP2',
    'FR_solicitorCreate'
  );
    ...
    if (!page.show_condition) {
      it('FR Consented Create Case Page ' + page.id, async function () {
        ...
        await pa11ytest(
          ...
          actions,
          conf.baseUrl +
            'cases/case-create/DIVORCE/FinancialRemedyMVP2/FR_solicitorCreate/' +
            page.id
        );
      });
    }
  ...
  const fr_ContestedConfig = ccdApi.getSolicitorCreateCaseConfig(
    ...
    if (!page.show_condition) {
      it('FR Contested Create Case Page ' + page.id, async function () {
        ...
        await pa11ytest(
          ...
          actions,
          conf.baseUrl +
            'cases/case-create/DIVORCE/FinancialRemedyContested/FR_solicitorCreate/' +
            page.id
        );
      });
    }
  ...
  const probateGrantOfrepresentation = ccdApi.getSolicitorCreateCaseConfig(
    'GrantOfRepresentation',
    'solicitorCreateApplication'
  );
  probateGrantOfrepresentation.wizard_pages.forEach((page) => {
    if (!page.show_condition) {
      it(
        'Probate Grant Of Representation Create Case Page ' + page.id,
        async function () {
          // await MockApp.startServer();
          const actions = [];
          actions.push(
          ...
          await pa11ytest(
            ...
            actions,
            conf.baseUrl +
              'cases/case-create/DIVORCE/GrantOfRepresentation/solicitorCreateApplication/' +
              page.id
          );
        }
      );
  ...
  const probateCreateCaveat = ccdApi.getSolicitorCreateCaseConfig(
    'Caveat',
    'solicitorCreateCaveat'
  );
  probateCreateCaveat.wizard_pages.forEach((page) => {
    if (!page.show_condition) {
      it(
        'Probate create caveat Create Case Page ' + page.id,
        async function () {
          // await MockApp.startServer();
          const actions = [];
          actions.push(
          ...
          await pa11ytest(
            ...
            actions,
            conf.baseUrl +
              'cases/case-create/DIVORCE/Caveat/solicitorCreateCaveat/' +
              page.id
          );
        }
      );
    ...
    if (!page.show_condition) {
      it('IA Create Case Page ' + page.id, async function () {
        ...
        await pa11ytest(
          ...
          actions,
          conf.baseUrl +
            'cases/case-create/DIVORCE/Asylum/startAppeal/' +
            page.id
        );
      });
    }
  ...
  const fplCareSupervisionConfig = ccdApi.getSolicitorCreateCaseConfig(
