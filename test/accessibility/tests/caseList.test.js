...
describe('Pa11y Accessibility tests', function () {
  ...
  it('Case List Page', async function () {
    ...
    const actions = [];
    actions.push(
      ...PallyActions.waitForPageWithCssLocator(
        'ccd-workbasket-filters .heading-h2'
      )
    );
    actions.push(...PallyActions.clickElement('ccd-workbasket-filters button'));

    actions.push(
      ...PallyActions.waitForPageWithCssLocator('pagination-template')
    );
  ...
  it('Share Case page', async function () {
    ...
    actions.push(
      ...PallyActions.waitForPageWithCssLocator(
        'exui-case-share #title-selected-cases'
      )
    );
    actions.push(
      ...PallyActions.waitForPageWithCssLocator(
        '#accordion-with-summary-sections .govuk-accordion__open-all span'
      )
    );

    actions.push(
      ...PallyActions.clickElement(
        '#accordion-with-summary-sections .govuk-accordion__open-all'
      )
    );

    actions.push(...PallyActions.inputField('xuilib-user-select input', '@'));
    actions.push(
      ...PallyActions.clickElement('.mat-autocomplete-visible mat-option span')
    );
    actions.push(
      ...PallyActions.clickElement('xuilib-share-case #btn-add-user')
    );
  ...
  it('Confirm Share a case page', async function () {
    ...
    actions.push(
      ...PallyActions.waitForPageWithCssLocator(
        'exui-case-share-confirm #summarySections'
      )
    );
    await pa11ytest(this, actions, conf.baseUrl + 'cases/case-share?init=true');
  });
  ...
  it('Share Case Submission success', async function () {
    ...
    actions.push(
      ...PallyActions.clickElement(
        'xuilib-share-case-confirm #share-case-nav button'
      )
    );
    actions.push(
      ...PallyActions.waitForPageWithCssLocator('.govuk-panel--confirmation')
    );
    await pa11ytest(this, actions, conf.baseUrl + 'cases/case-share?init=true');
  });
  ...
  it('Share Case Submission partial success', async function () {
    ...
    actions.push(
      ...PallyActions.waitForPageWithCssLocator('exui-case-share-complete')
    );
    await pa11ytest(this, actions, conf.baseUrl + 'cases/case-share?init=true');
  });
  ...
  it('Share Case Submission error page', async function () {
    ...
    actions.push(
      ...PallyActions.waitForPageWithCssLocator('exui-service-down')
    );
    await pa11ytest(this, actions, conf.baseUrl + 'cases/case-share?init=true');
  });
  ...
  function searchAndAddUserSteps() {
    const actions = [];
    actions.push(
      ...PallyActions.inputField('xuilib-user-select input', 'james')
    );
    actions.push(...PallyActions.clickElement('xuilib-user-select input'));
    actions.push(
      ...PallyActions.waitForPageWithCssLocator(
        '.mat-autocomplete-visible mat-option .mat-option-text'
      )
    );
    actions.push(
      ...PallyActions.clickElement(
