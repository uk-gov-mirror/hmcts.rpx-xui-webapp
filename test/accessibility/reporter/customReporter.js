...
function copyResources() {
  ...
  fs.copyFileSync(
    __dirname + '/resources/angular.min.js',
    resourceDir + 'angular.min.js'
  );
  fs.copyFileSync(__dirname + '/resources/css/all.css', cssDir + 'all.css');
  fs.copyFileSync(
    __dirname + '/resources/webfonts/fa-solid-900.woff2',
    webfontsDir + 'fa-solid-900.woff2'
  );
}
...
function consoleReport(reportjson) {
  ...
  for (
    let testCounter = 0;
    testCounter < reportjson.tests.length;
    testCounter++
  ) {
    ...
    if (test.status === 'failed') {
      ...
      if (a11yResult.issues) {
        for (
          let issueCounter = 0;
          issueCounter < a11yResult.issues.length;
          issueCounter++
        ) {
          console.log(
            '\t \t \t ' +
              (issueCounter + 1) +
              '. ' +
              a11yResult.issues[issueCounter].code
          );
