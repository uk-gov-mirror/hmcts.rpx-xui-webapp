  ...
  describe('Available tasks :', function () {
    [22, 0].forEach((tasksCount) => {
      it(`Display tasks count in Available tasks - showing ${tasksCount} tasks`, async function () {
        ...
        expect(
          await taskListPage.isAvailableTasksDisplayed(),
          'My tasks not dispplayed'
        ).to.be.true;

        expect(
          parseInt(await taskListPage.getTaskListCountInTable()),
        ...
        if (tasksCount === 0) {
          expect(
            await taskListPage.isTableFooterDisplayed(),
    ...
    it(`Sort tasks by column`, async function () {
      ...
      expect(
        await taskListPage.isAvailableTasksDisplayed(),
        'Available tasks not dispplayed'
      ).to.be.true;
      ...
      for (let i = 0; i < columnHeaders.length; i++) {
        ...
        const headerColId = await taskListPage
          .getHeaderElementWithName(headerName)
          .getAttribute('id');
        expect(await taskListPage.getColumnSortState(headerName)).to.equal(
          'none'
        );
    ...
    it(`Sort column persists in session`, async function () {
      ...
      const columnHeaders = await taskListPage.getColumnHeaderNames();
      console.log(columnHeaders);
      expect(await taskListPage.getColumnSortState(columnHeaders[1])).to.equal(
        'none'
      );
      ...
      for (const responseCode of testErrorResponseCodes) {
        ...
        MockUtil.setMockResponse(
          ...
          (req, res) => {
            res
              .status(responseCode)
              .send(workAllocationMockData.getMyTasks(10));
          }
        );
      ...
      for (const responseCode of testErrorResponseCodes) {
        ...
        MockUtil.setMockResponse(
          ...
          (req, res) => {
            res
              .status(responseCode)
              .send(workAllocationMockData.getAvailableTasks(10));
          }
        );
