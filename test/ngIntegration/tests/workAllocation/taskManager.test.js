  ...
  describe('Task manager :', function () {
    [22, 0].forEach((tasksCount) => {
      it(`Display tasks count in Task list - showing ${tasksCount} tasks`, async function () {
        await MockUtil.setMockResponse(
          'POST',
          '/workallocation/task/',
          (req, res) => {
        ...
        expect(await taskManagerPage.amOnPage(), 'My tasks not dispplayed').to
          .be.true;

        expect(
          parseInt(await taskManagerPage.getTaskListCountInTable()),
          'Task count does not match expected '
        ).to.equal(tasksCount);
        expect(
          parseInt(await taskManagerPage.getTaskCountInDisplayLabel()),
          'Task count does not match expected '
        ).to.equal(tasksCount);
        if (tasksCount === 0) {
          expect(
            await taskManagerPage.isTableFooterDisplayed(),
            'task list table footer is not displayed'
          ).to.be.true;
          expect(
          ...
          expect(
            await taskManagerPage.isTableFooterDisplayed(),
            'task list table footer is displayed'
          ).to.be.false;
        }
      });
    });
    ...
    it(`Sort tasks by column`, async function () {
      ...
      expect(
        await taskManagerPage.amOnPage(),
        'Task manager page not dispplayed'
      ).to.be.true;
      ...
      for (let i = 0; i < columnHeaders.length; i++) {
        ...
        const headerColId = await taskManagerPage
          .getHeaderElementWithName(headerName)
          .getAttribute('id');
        expect(await taskManagerPage.getColumnSortState(headerName)).to.equal(
          'none'
        );
    ...
    it(`Sort column persists in session`, async function () {
      await navigatetoTaskManagerPage();
      expect(await taskManagerPage.amOnPage(), 'Task manager not dispplayed').to
        .be.true;

      const columnHeaders = await taskManagerPage.getColumnHeaderNames();
      console.log(columnHeaders);
      expect(
        await taskManagerPage.getColumnSortState(columnHeaders[1])
      ).to.equal('none');
    ...
    it(`Task manager on error `, async function () {
      ...
      for (const responseCode of testErrorResponseCodes) {
        ...
        MockUtil.setMockResponse(
          'POST',
          '/workallocation/task/',
          (req, res) => {
            res
              .status(responseCode)
              .send(workAllocationMockData.getTaskManagerTasks(10));
          }
        );
        ...
        await softAssertion.assert(
          async () =>
            expect(
              ...
              'Error page not displayed on error ' + responseCode
            ).to.be.true
        );
        if (isErrorPageDisplayed) {
          ...
          await softAssertion.assert(async () =>
            expect(
              ...
              'Error message does not match on error ' + responseCode
      ...
      for (const responseCode of testErrorResponseCodes) {
        ...
        await softAssertion.assert(
          async () =>
            expect(
              isErrorPageDisplayed,
              '/workallocation/location on error, error page not displayed ' +
                responseCode
            ).to.be.true
        );
        if (isErrorPageDisplayed) {
          ...
          await softAssertion.assert(async () =>
            expect(
              errorMessageDisplayed,
              '/workallocation/location on error,Error message does not match ' +
                responseCode
            ).to.contains(errorMessageForResponseCode(responseCode))
          );
        }
      ...
      for (const responseCode of testErrorResponseCodes) {
        ...
        await softAssertion.assert(
          async () =>
            expect(
              isErrorPageDisplayed,
              '/workallocation/caseworker on error, error page not displayed ' +
                responseCode
            ).to.be.true
        );
        if (isErrorPageDisplayed) {
          ...
          await softAssertion.assert(async () =>
            expect(
              errorMessageDisplayed,
              '/workallocation/caseworkeron error,Error message does not match ' +
                responseCode
            ).to.contains(errorMessageForResponseCode(responseCode))
          );
        }
