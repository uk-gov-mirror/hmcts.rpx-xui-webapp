...
describe('Unhappy path task actions: ', function () {
  ...
  it(`My Tasks - Reassign page errors`, async function () {
    ...
    const reassignEndpoints = [
      ...
      {
        name: 'caseworkers',
        url: '/workallocation/caseworker/location/:locationId',
      },
    ];
    for (const endPoint of reassignEndpoints) {
      for (const responseCode of testErrorResponseCodes) {
        ...
        expect(
          ...
          'Default My tasks tab page not displayed'
        ).to.be.true;

        await tasklistPage.clickManageLinkForTaskAt(1);
        expect(
          await tasklistPage.isTaskActionRowForTaskDisplayed(1),
          'Task actions for selected task not displayed'
        ).to.be.true;
  ...
  it(`My Tasks - Reassign submit errors`, async function () {
    ...
    for (const responseCode of testErrorResponseCodes) {
      ...
      await tasklistPage.clickTaskAction('Reassign task');
      expect(await taskAssignmentPage.amOnPage(), 'Not on task assignment page')
        .to.be.true;
      ...
      setErrorRespondeCodeOnApi(
        'POST',
        '/workallocation/task/:taskId/assign',
        responseCode
      );
  ...
  it(`Available - action link page errors`, async function () {
    ...
    for (const action of availableTask_actions) {
      for (const responseCode of testErrorResponseCodes) {
        ...
        await headerPage.clickManageCases();
        await setErrorRespondeCodeOnApi(
          'GET',
        ...
        expect(
          ...
          'Available tasks tab page not displayed'
        ).to.be.true;
