...
describe('CCD casefields, retain_hidden_field setting', function () {
  ...
  afterEach(async function (done) {
    console.log(
      this.test.ctx.currentTest.title + ' : ' + this.test.ctx.currentTest.state
    );
    await MockApp.stopServer();
    if (this.test.ctx.currentTest.state === 'failed') {
      addContext(this, {
        title: 'Case Event create/edit config',
        value: caseEventConfigOfTest,
      });
      addContext(this, {
        title: 'Case Event submit request body',
        value: caseEventSubmitRequestBody,
      });
    }
    done();
  });
  ...
  [
    ...
    it(`Text Field : retain_hidden_field "${scenario.retainHiddenField}" and field is Displayed ${scenario.showField}`, async function () {
      ...
      const caseConfig = new CCDCaseConfig(
        'TEST_CaseType',
        'Test case type hidden field retain value',
        'test description'
      );
      const page1 = caseConfig.addWizardPage(
        'HiddenFieldPage_1',
        'Hidden field retain value test page'
      );

      let testFieldShowYesNo = caseConfig.addCCDFieldToPage(
        page1,
        'YesOrNo',
        'showTestField',
        'Show Test Field?'
      ...
      let textField = caseConfig.addCCDFieldToPage(
        page1,
        'Text',
        'simpleTextField',
        'Test old value'
      ...
      await browser.get(
        `cases/case-details/1604309496714935/trigger/casetype_1/HiddenFieldPage_1`
      );
      ...
      if (!scenario.showField && scenario.retainHiddenField === 'yes') {
        expect(caseEventSubmitRequestBody.data).to.not.have.property(
          textField.id
        );
      }
      ...
      if (scenario.showField && scenario.retainHiddenField === 'No') {
        expect(caseEventSubmitRequestBody.data).to.have.property(textField.id);
        expect(caseEventSubmitRequestBody.data[textField.id]).to.equal(
          'Test old value'
        );
      }
    });
  ...
  [
    ...
    {
      fieldType: 'AddressGlobalUK',
      showField: false,
      retainHiddenField: false,
    },
    ...
    it(`field ${scenario.fieldType} : retain_hidden_field "${scenario.retainHiddenField}" and field is Displayed ${scenario.showField}`, async function () {
      ...
      switch (scenario.fieldType) {
        ...
        case 'Document':
          complexField.value = {
            document_url:
              'http://dm-store-aat.service.core-compute-aat.internal/documents/a612199d-9972-4b99-b653-5ec7c310e21a',
            document_binary_url:
              'http://dm-store-aat.service.core-compute-aat.internal/documents/a612199d-9972-4b99-b653-5ec7c310e21a/binary',
            document_filename:
              'Redacted-dm-store135044941749889827327305460282199740737.pdf',
          };
          break;
        default:
          throw new Error(
            'Unrecognised comples filed type ' + scenario.fieldType
          );
      }
      setUpcaseConfig(caseConfig.caseConfigTemplate);
      // CCD case config setup end
      ...
      if (!scenario.showField && scenario.retainHiddenField === 'No') {
        expect(caseEventSubmitRequestBody.data).to.have.property(
          complexField.id
        );
        expect(
          validateAllValuesNullInObject(
            caseEventSubmitRequestBody.data[complexField.id]
          ),
          'object has non null values'
        ).to.be.true;
      }
    ...
    it(`field ${scenario.fieldType} : retain_hidden_field "${scenario.retainHiddenField}" and field is Displayed ${scenario.showField}`, async function () {
      ...
      switch (scenario.fieldType) {
        case 'Collection':
          ...
          const collectionFieldItem = caseConfig.getCCDFieldTemplateCopy(
            'Text',
            'CollText',
            'Collection Text'
          );
          complexCollectionField.field_type.complex_fields = [
            collectionFieldItem,
          ];
          complexField.field_type.collection_field_type =
            complexCollectionField.field_type;
          complexField.value = [
        ...
        default:
          throw new Error(
            'Unrecognised list/collection field type provided for test'
          );
      }
  ...
  [
    ...
    it(`Deep complex field : ${scenario.testName}`, async function () {
      ...
      const parentComplexField = caseConfig.addCCDFieldToPage(
        page1,
        'Complex',
        'parentComplexField',
      ...
      parentComplexField.retain_hidden_value =
        scenario.parentComplex.retainValue;
      parentComplexField.show_condition = `${parentComplexShowYesNo.id}=\"Yes\"`;

      //level 1 fields
      const childTextShowCondition = caseConfig.getCCDFieldTemplateCopy(
        'YesOrNo',
        'childTextShowCondition',
        'Show child text?'
      );
      childTextShowCondition.value = true;
      const childText = caseConfig.getCCDFieldTemplateCopy(
        'Text',
        'childText',
        'Child Text field'
      );
      ...
      const childComplex = caseConfig.getCCDFieldTemplateCopy(
        'Complex',
        'childComplex',
        'Child Complex field'
      );
      ...
      const grandchildText = caseConfig.getCCDFieldTemplateCopy(
        'Text',
        'grandchildText',
        'Grandchild Text'
      );
      ...
      childComplex.field_type.complex_fields = [
        grandchildTextShowCindition,
        grandchildText,
      ];
      //level 2 fields end

      parentComplexField.field_type.complex_fields = [
        childTextShowCondition,
        childText,
        childComplexShowCondition,
        childComplex,
      ...
      parentComplexFieldValue[childComplex.id][
        grandchildTextShowCindition.id
      ] = true;
      parentComplexFieldValue[childComplex.id][grandchildText.id] =
        'Grand child Text Value old';
      ...
      await CCDCaseEditPage.inputCaseField(
        parentComplexField.id + '_' + childText.id,
      ...
      await CCDCaseEditPage.selectRadioYesOrNo(
        parentComplexShowYesNo.id,
        scenario.parentComplex.display
      );
      ...
      await browser.executeScript(
        ...
        CCDCaseEditPage.getSubmitButton().getWebElement()
      );
      await CCDCaseEditPage.clickSubmit();
      await BrowserWaits.waitForCondition(async () => {
        return !(await CCDCaseEditPage.isCheckYourAnswersPagePresent());
      ...
      expect(
        caseEventSubmitRequestBody.data[parentComplexField.id]
      ).to.have.property(childText.id);
      expect(
        caseEventSubmitRequestBody.data[parentComplexField.id]
      ).to.have.property(childComplex.id);
      expect(
        caseEventSubmitRequestBody.data[parentComplexField.id][childComplex.id]
      ).to.have.property(grandchildText.id);

      if (scenario.parentComplex.display && scenario.childText.display) {
        expect(
          caseEventSubmitRequestBody.data[parentComplexField.id][childText.id],
          'For field' + childText.id
        ).to.equal('Child text value new');
      } else if (scenario.childText.retainValue) {
        expect(
          caseEventSubmitRequestBody.data[parentComplexField.id][childText.id],
          'For field' + childText.id
        ).to.equal('Child text value old');
      } else {
        expect(
          caseEventSubmitRequestBody.data[parentComplexField.id][childText.id],
          'For field' + childText.id
  ...
  function setUpcaseConfig(caseConfig) {
    MockApp.onGet(
      '/data/internal/cases/:caseid/event-triggers/:eventId',
      (req, res) => {
    ...
    MockApp.onPost('/data/case-types/:caseType/validate', (req, res) => {
      ...
      const responseBody = {
        ...
        _links: {
          self: {
            href:
              'http://ccd-data-store-api-aat.service.core-compute-demo.internal' +
              req.path +
              '?pageId=' +
              req.query.pageId,
          },
