...
describe('CCD-CASE-UI-TOOLKIT collection field permissions', function () {
  ...
  const addNewButton = element(
    by.xpath("//div[@id = 'textCollection']//button[text() = 'Add new']")
  );
  const removeButton = element(
    by.xpath("//div[@id = 'textCollection']//button[text() = 'Remove']")
  );
  ...
  it('display_context_parameter combination of conditions', async function () {
    setUpcaseConfig(
      '#TABLE(AddressLine1,AddressLine2),#COLLECTION(allowInsert,allowDelete)'
    );
