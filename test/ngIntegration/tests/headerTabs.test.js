...
describe('Header  Tabs', function () {
  ...
  it('Case list tab present', async function () {
    BrowserUtil.setUserDetailsWithRoles([
    ...
    expect(
      ...
      'Case list tab not present, displayed tabs : '
    ).to.be.true;
  });
  ...
  it('Create Case tab present', async function () {
    ...
    expect(
      ...
      'Create case tab not present, displayed tabs : '
    ).to.be.true;
  });
  ...
  it('Work allocation Tabs not present without roles "caseworker-ia-admofficer" and "caseworker-ia-caseofficer"', async function () {
    ...
    expect(
      ...
      'Task list tab not present, displayed tabs : ' +
        (await headerPage.primaryNavBar.getText())
    ).to.be.false;
    expect(
      await headerPage.isTabPresent('Task manager'),
      'Task manager tab present, displayed tabs : ' +
        (await headerPage.primaryNavBar.getText())
    ).to.be.false;
  });
  ...
  it('Work allocation Tabs "Task list" and "Task manager" for role "caseworker-ia-caseofficer"', async function () {
    ...
    expect(
      await headerPage.isTabPresent('Task manager'),
      'Task manager tab not present, displayed tabs : ' +
        (await headerPage.primaryNavBar.getText())
    ).to.be.true;
    expect(
      await headerPage.isTabPresent('Task list'),
