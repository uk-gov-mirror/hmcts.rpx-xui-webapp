...
describe('CCD Endpoints', () => {
  ...
  it('Jurisdictions for user role', async function () {
    ...
    expect(response.data.map((e) => e.name)).to.include.members([
      'Family Divorce',
      'Public Law',
      'Immigration & Asylum',
  ...
  for (const jurisdiction of jurisdictions) {
    for (const caseType of jurisdiction.caseTypeIds) {
      it(`work-basket-input for casetype  ${caseType}`, async () => {
        await Request.withSession(userName, password);
        const response = await Request.get(
          `data/internal/case-types/${caseType}/work-basket-inputs`,
          { experimental: true }
        );
        expect(response.status).to.equal(
          200,
          `request with ${caseType} failed`
        );
  ...
  for (const jurisdiction of jurisdictions) {
    for (const caseType of jurisdiction.caseTypeIds) {
      it(`Cases pagination metadata for casetype  ${caseType}`, async () => {
        await Request.withSession(userName, password);
        const response = await Request.get(
          `data/caseworkers/:uid/jurisdictions/${jurisdiction.id}/case-types/${caseType}/cases/pagination_metadata`,
          null
        );
        expect(response.status).to.equal(
          200,
  ...
  async function getCasesForCaseType(jurisdiction: string, casetype: string) {
    ...
    const casesResponse = await Request.get(
      `data/internal/searchCases?ctid=${casetype}&use_case=WORKBASKET&view=WORKBASKET&state=Any`,
      headers
    );
