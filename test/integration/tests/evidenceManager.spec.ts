...
describe('Evidence Manager Endpoints', () => {
  ...
  it('Get document binary', async function () {
    ...
    const response = await Request.get(
      `documents/${config.em.docId}/binary`,
      headers
    );
  ...
  it('Get document annotations metadata', async () => {
    ...
    const response = await Request.get(
      `em-anno/metadata/${config.em.docId}`,
      headers
    );
  ...
  it('Get document annotations filter', async () => {
    ...
    const response = await Request.get(
      `em-anno/annotation-sets/filter?documentId=${config.em.docId}`,
      null
    );
    expect(response.status).to.equal(200);
  ...
  it('Delete document annotation', async () => {
    ...
    if (annotationsRes.data.annotations.length <= 1) {
      const newannoRes = await Request.put(
        `em-anno/annotations`,
        await getAnnotationObject(),
        headers
      );
    ...
    const response = await Request.delete(
      `em-anno/annotations/${annoIdToDelete}`,
      await getAnnotationObject(),
      headers
    );
    expect(response.status).to.equal(200);
  ...
  it('Get document bookmarks', async () => {
    ...
    const response = await Request.get(
      `em-anno/${config.em.docId}/bookmarks`,
      null
    );
    expect(response.status).to.equal(200);
  ...
  it('Put document bookmark', async () => {
    ...
    const bookmarksCountBefore = await Request.get(
      `em-anno/${config.em.docId}/bookmarks`,
      null
    );
    ...
    const response = await Request.put(
      `em-anno/bookmarks`,
      await getNewBookmarkIdObject('test123', config.em.docId, 0, null),
      headers
    );
    expect(response.status).to.equal(200);
  ...
  it('Delete document bookmark', async () => {
    ...
    const bookmarksgetResponse = await Request.get(
      `em-anno/${config.em.docId}/bookmarks`,
      null
    );
