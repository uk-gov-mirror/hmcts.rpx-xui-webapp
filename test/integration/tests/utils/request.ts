...
class Request {
  ...
  public async get(reqpath: string, headers: any) {
    try {
      return await this.retryRequest(() =>
        http.get(reqpath, this.getRequestConfig(headers))
      );
    } catch (error) {
  ...
  public async post(reqpath: string, data, headers: any) {
    try {
      return await this.retryRequest(() =>
        http.post(reqpath, data, this.getRequestConfig(headers))
      );
    } catch (error) {
      return this.getResponseFromError(error);
    }
  ...
  public async put(reqpath: string, data, headers: any) {
    try {
      return await this.retryRequest(() =>
        http.put(reqpath, data, this.getRequestConfig(headers))
      );
    } catch (error) {
      return this.getResponseFromError(error);
    }
  ...
  public async retryRequest(callback) {
    ...
    while (retryAttemptCounter < 3 && !retVal) {
      try {
        ...
        retryAttemptCounter++;
        console.log(
          `API request error: "${err.code}" syscall : ${err.syscall} => Retrying atempt ${retryAttemptCounter}`
        );
        if (err.response) {
          retVal = err.response;
        }
    ...
    if (retVal) {
      ...
      throw new Error(
        'Errors occured in retry attempts ' + JSON.stringify(retryErrorLogs)
      );
    }
  }
}
