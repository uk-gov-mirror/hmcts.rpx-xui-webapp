...
export async function getSessionCookieString(username, password) {
  if (!authCookiesForUsers.hasOwnProperty(username)) {
    authCookiesForUsers[username] = await authenticateAndGetcookies(
      username,
      password
    );
  }
  ...
  while (loginAttemptsCounter < 5 && !isLoginSuccess) {
    try {
      ...
      while (waitCounter < 20) {
        await setTimeout(() => {
          console.log(
            waitCounter + '  : timeout sleep ' + new Date().toTimeString()
          );
        }, 1000);
        waitCounter++;
        if (primaryNavElement != undefined) {
          ...
          const usernameInput = await page.$eval(
            '#username',
      ...
      await page.waitForSelector('.hmcts-primary-navigation', {
        visible: true,
        timeout: 10000,
      });
