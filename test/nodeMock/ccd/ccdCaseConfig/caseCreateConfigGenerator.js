const configTemplate = {
  ...
  _links: {
    self: {
      href:
        'http://gateway-ccd.aat.platform.hmcts.net/internal/case-types/FinancialRemedyContested/event-triggers/FR_solicitorCreate?ignore-warning=false',
    },
  },
};
...
const wizardPageTemplate = {
  ...
  callback_url_mid_event:
    'http://finrem-cos-aat.service.core-compute-aat.internal/case-orchestration/contested/set-defaults',
  ...
  getCCDFieldTemplateCopy(fieldType, id, label) {
    ...
    switch (fieldType) {
      ...
      case 'AddressUK':
        ...
        const add1 = this.getCCDFieldTemplateCopy(
          'Text',
          'AddressLine1',
          'Building and Street'
        );
        const add2 = this.getCCDFieldTemplateCopy(
          'Text',
          'AddressLine2',
          'Address Line 2'
        );
        const add3 = this.getCCDFieldTemplateCopy(
          'Text',
          'AddressLine3',
          'Address Line 3'
        );
        const postTown = this.getCCDFieldTemplateCopy(
          'Text',
          'PostTown',
          'Town or City'
        );
        const county = this.getCCDFieldTemplateCopy('Text', 'County', 'County');
        const postCode = this.getCCDFieldTemplateCopy(
          'Text',
          'PostCode',
          'Postcode/Zipcode'
        );
        const country = this.getCCDFieldTemplateCopy(
          'Text',
          'Country',
          'Country'
        );

        template.field_type.complex_fields = [
          add1,
          add2,
          add3,
          postTown,
      ...
      case 'Organisation':
        ...
        const organisation = this.getCCDFieldTemplateCopy(
          'Complex',
          'Organisation',
          'Organisation'
        );
        organisation.field_type.id = 'Organisation;';
        const orgId = this.getCCDFieldTemplateCopy(
          'Text',
          'OrganisationID',
          'Organisation ID'
        );
        const orgNaMe = this.getCCDFieldTemplateCopy(
          'Text',
          'OrganisationName',
          'Name'
        );
