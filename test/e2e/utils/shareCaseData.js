...
class ShareCaseData {
  ...
  MarkUserToShare(caseid, email) {
    let caseData = this.getCaseWithId(caseid);
    if (
      caseData.sharedWith.indexOf(email) === -1 &&
      caseData.markedForShare.indexOf(email) === -1
    ) {
      console.log(
        '******* Test data Tracking: => Marking TO BE ADDED : ' +
          caseid +
          ' : ' +
          email
      );
  ...
  MarkUserToUnShare(caseid, email) {
    let caseData = this.getCaseWithId(caseid);
    console.log(
      '******* Test data Tracking: => Marking TO BE REMOVED : ' +
        caseid +
        ' : ' +
        email
    );
  ...
  CancelMarkedForShare(caseid, email) {
    ...
    if (index > -1) {
      console.log(
        '******* Test data Tracking: => cancel TO BE ADDED : ' +
          caseid +
          ' : ' +
          email
      );
  ...
  CancelMarkedForUnShare(caseid, email) {
    ...
    if (index > -1) {
      console.log(
        '******* Test data Tracking: => cancel TO BE REMOVED : ' +
          caseid +
          ' : ' +
          email
      );
  ...
  changesCommited() {
    for (let i = 0; i < cases.length; i++) {
      let caseData = cases[i];
      for (
        let sharedWithUser = 0;
        sharedWithUser < caseData.markedForShare.length;
        sharedWithUser++
      ) {
      ...
      for (
        let sharedWithUser = 0;
        sharedWithUser < caseData.markedForUnShare.length;
        sharedWithUser++
      ) {
