...
defineSupportCode(function ({ And, But, Given, Then, When }) {
  ...
  Then('I see Share Case check and confirm page', async function () {
    expect(
      await shareCaseCheckAndConfirmPage.amOnPage(),
      'Share Case Check and confirm page not displayed'
    ).to.be.true;
  });

  Then(
    'I see Share Case changes are listed as modified in share case page',
    async function () {
      let issues = await shareCaseCheckAndConfirmPage.validateShareCaseChangesForListedCases();
      expect(
        issues.length,
        'Share Case check and confirm not matching expected : ' +
  ...
  When(
    'I click back link in Share Case check and confirm page',
    async function () {
  ...
  When(
    'I click change link for case at pos {int} in Share Case check and confirm page',
    async function (casePos) {
      await shareCaseCheckAndConfirmPage.clickChangeLinkForCase(casePos);
    }
  );

  When(
    'I click Confirm button in Share Case check and confirm page',
    async function () {
  ...
  Then('I see success message for Share case changes', async function () {
    expect(
      await shareCaseCheckAndConfirmPage.isSubmissionSuccessful(),
      'Share Case changes submission is not successful'
    ).to.be.true;
  });
});
