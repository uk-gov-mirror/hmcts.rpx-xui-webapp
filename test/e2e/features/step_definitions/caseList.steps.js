...
defineSupportCode(function ({ And, But, Given, Then, When }) {
  ...
  Then(
    'I wait to see case results displayed',
    { timeout: 120 * 1000 },
    async function () {
  ...
  Then(
    'I see case list table header has Select all checkbox column',
    async function () {
      expect(
        await caseListPage.isSelectAllColumnDisplayed(),
        'Case list selection Select all column/checkbox not present '
      ).to.be.true;
    }
  );

  Then(
    'I see case list table header does not have Select all checkbox column',
    async function () {
      expect(
        await caseListPage.isSelectAllColumnDisplayed(),
        'Case list selection Select all column/checkbox not present '
  ...
  Then(
    'I see case list table each case row has checkbox column',
    async function () {
      expect(
        await caseListPage.isSelectCheckboxDisplayedForAllCases(),
        'Case select checkbox not displayed for all acses'
      ).to.be.true;
    }
  );
  ...
  When(
    'I unselect case at row {int} in case list page',
    async function (rownum) {
      if (await caseListPage.isCaseSelectCheckboxSelected(rownum)) {
        await caseListPage.clickCaseSelectCheckBoxAtRow(rownum);
      } else {
  ...
  Then(
    'I see case list table has case at row {int} selected',
    async function (rowNum) {
      expect(
        await caseListPage.isCaseSelectCheckboxSelected(rowNum),
        'Case select checkbox not selected for case at row ' + rowNum
      ).to.be.true;
    }
  );

  Then(
    'I see case list table has case at row {int} un-selected',
    async function (rowNum) {
      expect(
        await caseListPage.isCaseSelectCheckboxSelected(rowNum),
        'Case select checkbox still selected for case at row ' + rowNum
      ).to.be.false;
    }
  );
  ...
  Then(
    'I see case list table has {int} rows selected',
    async function (selectedCasesCount) {
      expect(
        await caseListPage.getCountOfCasesSelectedInPage(),
        'Selected cases count does not match expected count' +
          selectedCasesCount
  ...
  When(
    'I sort case list table by column at position {int}',
    async function (colPos) {
      await caseListPage.sortTableByColAt(colPos);
    }
  );

  When(
    'I click case at row {int} and navigate to case view page',
    async function (rowNum) {
  ...
  Then(
    'I see case list selection feature {string} available',
    async function (featureCondition) {
      ...
      if (featureCondition.toLowerCase() === 'is') {
        expect(
          await caseListPage.tableHeaderSelectAllInput.isPresent(),
          'Table header Select All check box not present'
        ).to.be.true;
        expect(
          await caseListPage.shareCaseButton.isPresent(),
          'Share Case button not present'
        ).to.be.true;
        expect(
          await caseListPage.resetCaseSelectionLink.isPresent(),
          'Reset selection button not present'
        ).to.be.true;
      } else {
        expect(
          await caseListPage.tableHeaderSelectAllInput.isPresent(),
          'Table header Select All check box should not be  present'
        ).to.be.false;
        expect(
          await caseListPage.shareCaseButton.isPresent(),
          'Share Case button should not be  present'
        ).to.be.false;
        expect(
          await caseListPage.resetCaseSelectionLink.isPresent(),
          'Reset selection button should not be  present'
        ).to.be.false;
      }
    }
  );
