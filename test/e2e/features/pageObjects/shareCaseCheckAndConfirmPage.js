...
class ShareCaseCheckAndConfirmPage {
  constructor() {
    ...
    this.selectedCaseConfirmList = $$(
      'xuilib-selected-case-confirm #user-access-block'
    );
    this.backLink = $('.govuk-back-link');
    this.confirmBtn = $('#share-case-nav button');

    this.changesSubmissionConfirmationContainer = $(
  ...
  async waitForPageToLoad() {
    await BrowserWaits.waitForElement(
      this.pageContainer,
      'Share Case Conform your selection page not displayed.'
    );
    await BrowserWaits.waitForElement(
      this.summarySelectionContainer,
      'Share Case confirm selection summary not displayed'
    );
  }
  ...
  async getCaseIdOfCaseContainer(containerIndex) {
    ...
    await BrowserWaits.waitForElement(
      caseContainer.$(
        '.case-share-confirm__caption-area .case-share-confirm__caption'
      ),
      'case sub title not displayed for case at pos ' + containerIndex
    );
    let caseId = await caseContainer
      .$('.case-share-confirm__caption-area .case-share-confirm__caption')
      .getText();
    return caseId;
  }
  ...
  async isUserMarkedToBeRemovedIncase(caseId, email) {
    ...
    return userActionStatus
      ? userActionStatus.toLowerCase().includes('remove')
      : false;
  }
  ...
  async isUserMarkedToBeAddedIncase(caseId, email) {
    let userActionStatus = await this.getUserActionForCaseId(caseId, email);
    return userActionStatus
      ? userActionStatus.toLowerCase().includes('added')
      : false;
  }
