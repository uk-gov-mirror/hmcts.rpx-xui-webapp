...
class CaseListPage {
  constructor() {
    ...
    this.searchApplyBtn = $(
      'ccd-workbasket-filters form button:not(.button-secondary)'
    );
  ...
  async selectJurisdiction(jurisdiction) {
    ...
    await this.jurisdictionSelectElement
      .element(this._getOptionSelectorWithText(jurisdiction))
      .click();
  }

  async selectCaseType(caseType) {
    await this._waitForSearchComponent();
    await this.caseTypeSelectElement
      .element(this._getOptionSelectorWithText(caseType))
      .click();
  }

  async selectState(state) {
    await this._waitForSearchComponent();
    await this.stateSelectElement
      .element(this._getOptionSelectorWithText(state))
      .click();
  }
  ...
  async waitForCaseResultsToDisplay() {
    ...
    CucumberReportLogger.AddMessage(
      'starting wait for 2 sec for list to render  : ' +
        new Date().toTimeString()
    );
    await BrowserWaits.waitForSeconds(2);
    CucumberReportLogger.AddMessage(
      'wait complete : ' + new Date().toTimeString()
    );
  }
  ...
  async clickPaginationNextPage() {
    ...
    expect(
      await this.nextPageLink.isPresent(),
      'Case list next page not present. current page info : ' +
        paginationInfobefore
    ).to.be.true;
    ...
    await BrowserWaits.waitForElement(
      this.paginationControlsContainer,
      undefined,
      'Data load taking too long on pagination'
    );
  }
  ...
  async clickPaginationPreviousPage() {
    let paginationInfobefore = await this.paginationInfotext.getText();
    expect(
      await this.previousPageLink.isPresent(),
      'Case list previous page not present. current page info : ' +
        paginationInfobefore
    ).to.be.true;
  ...
  async clickCaseLinkAtRow(rowNum) {
    ...
    await BrowserWaits.waitForElement(
      this.ccdCaseViewer,
      undefined,
      'Case view page is not displayed'
    );
  }
