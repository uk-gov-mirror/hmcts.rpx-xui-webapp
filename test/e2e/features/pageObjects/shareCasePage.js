...
class ShareCasePage {
  ...
  async testData_storeCaseShareDataOnPage() {
    ...
    for (let caseCounter = 1; caseCounter <= casesCount; caseCounter++) {
      ...
      for (
        let userCounter = 1;
        userCounter <= usersCountInCase;
        userCounter++
      ) {
        let email = await this.getEmailForUserIncase(caseCounter, userCounter);
        let actionLinktext = await this.getActionLinkTextForUser(
          caseCounter,
          userCounter
        );
        if (actionLinktext === 'Cancel') {
          let actionStatusLabel = await this.getActionLabelForUserWithEmail(
            caseCounter,
            email
          );
      ...
      ShareCaseData.AddCaseShareData(
        caseId,
        sharedWith,
        tobeAdded,
        tobeRemoved
  ...
  async getFilteredUserEmails() {
    ...
    for (
      let userCounter = 0;
      userCounter < usernameEmails.length;
      userCounter++
    ) {
  ...
  async getFilteredUserNameEmails() {
    ...
    CucumberReportLog.AddMessage(
      'Filtered Users : ' + JSON.stringify(userNameEmails)
    );
  ...
  async selectUserFromFilteredList(userNum) {
    ...
    CucumberReportLog.AddMessage(
      'Selected User : ' + (await userToSelect.getText())
    );
  ...
  async clickAddUserbutton() {
    ...
    this.testData_lastSelectedUser = '';
    CucumberReportLog.AddMessage(
      'Click Add with user selected : ' + this.testData_lastAddedUser
    );
  ...
  async clickDeselectCase(caseNum) {
    let selectedCase = await this.selectedCases.get(caseNum - 1);
    CucumberReportLog.AddMessage(
      'Deselecting Case ' + (await selectedCase.getText())
    );
  ...
  async clickCaseDetailsExpandCollapseBtn(caseNum) {
    ...
    await BrowserWaits.waitForElement(
      button,
      'Expand/collapse icon not present'
    );
  ...
  async getActionLinkTextForUser(caseNum, userNum) {
    ...
    let linkText = await browser.executeScript(
  ...
  async clickActionLinkForUser(caseNum, userNum) {
    ...
    let actionLinkLabel = await this.getActionLabelForUserWithEmail(
      caseNum,
      userEmail
    );
    ...
    if (actionLinktext === 'Remove') {
      ...
      CucumberReportLog.AddMessage(
        caseid + ' Case user uis marked for Unshare ' + userEmail
      );
      console.log(
        '*********** Marking to Remove ' + caseid + ' : email ' + userEmail
      );
    } else if (actionLinkLabel.includes('added')) {
      ShareCaseData.CancelMarkedForShare(caseid, userEmail);
      CucumberReportLog.AddMessage(
        caseid + ' Case user uis marked for Share ' + userEmail
      );
      console.log(
        '*********** Cancel to be Added ' + caseId + ' : email ' + userEmail
      );
    } else {
      ShareCaseData.CancelMarkedForUnShare(caseid, userEmail);
      CucumberReportLog.AddMessage(
        caseid + ' canceled Marked or Unshare ' + userEmail
      );
      console.log(
        '*********** Cancel to be removed ' + caseId + ' : email ' + userEmail
      );
    }
  }
  ...
  async getEmailForUserIncase(caseNum, userNum) {
    ...
    let email = await browser.executeScript(
      'return arguments[0].textContent',
  ...
  async getActionStatusLabelForUser(caseNum, userNum) {
    ...
    let actionLabel = await browser.executeScript(
      'return arguments[0].textContent',
      actionLabelCol
    );
    return actionLabel;
  ...
  async getUserRowInCase(caseNum, email) {
    ...
    for (let user = 0; user < userCount; user++) {
      ...
      let userEmail = await browser.executeScript(
        'return arguments[0].textContent',
        userRow.$('td:nth-of-type(2)')
      );
  ...
  async clickOpenOrCloseAllLink() {
    await BrowserWaits.waitForElement(
      this.openCloseAll,
      'OpenAll/CloaseAll button not present'
    );
  ...
  async selectUserWithEmail_Not_SharedWithAtLeastOneCase() {
    ...
    for (let i = 0; i < useremails.length; i++) {
      email = useremails[i];
      let caseNum = await this.isUserWithEmailNotSharedWithAtleastOneCase(
        email
      );
      if (caseNum > 0) {
        userToSelect = i + 1;
    ...
    if (userToSelect > 0) {
      ...
      CucumberReportLog.AddMessage(
        'Selected a user not shared with any case : ' + email
      );
    } else {
      throw Error(
  ...
  async selectUserWithEmail_SharedWithAtLeastOneCase() {
    ...
    if (userToSelect > 0) {
      ...
      this.testData_lastSelectedUser = email;
      CucumberReportLog.AddMessage(
        'Selected a user  shared with atleast one case : ' + email
      );
    } else {
      throw Error(
  ...
  async isLastAddedUserListedInAllCases() {
    ...
    for (let i = 1; i <= casesCount; i++) {
      if (
        !(await this.isUserWithEmailListedInCase(
          i,
          this.testData_lastAddedUser
        ))
      ) {
  ...
  async isLastAddedUserMarkedTobeAddedInAnyCase() {
    ...
    for (let i = 1; i <= casesCount; i++) {
      if (
        await this.isUserWithEmailMarkedToBeAdded(
          i,
          this.testData_lastAddedUser
        )
      ) {
  ...
  async isAnyUserMarkedToBeRemoved() {
    ...
    for (let i = 1; i <= casesCount; i++) {
      ...
      for (
        ...
        let userActionLabel = await this.getActionStatusLabelForUser(
          i,
          userCounter
        );
  ...
  async clickRemoveForAUserInListedCases() {
      ...
      for (
        ...
        if (actionLinktext.toLowerCase().includes('remove')) {
          if (!(await this.isCaseContentDisplayed(caseCounter))) {
            CucumberReportLog.AddMessage(
              'Expanding Case at pos ' + caseCounter
            );
            await this.clickCaseDetailsExpandCollapseBtn(caseCounter);
          }
          CucumberReportLog.AddMessage(
            'Click Remove link for  case pos' +
              caseCounter +
              ' user pos ' +
              userCounter
          );
    ...
    throw Error(
      'No cases have cases already shared or not marked to be removed. Cannot proceed with scenario.'
    );
  }
  ...
  async validateShareCaseChangesPersisted() {
    ...
    for (let caseCounter = 1; caseCounter <= casesCount; caseCounter++) {
      ...
      for (
        let shareWithCounter = 0;
        shareWithCounter < caseShareData.sharedWith.length;
        shareWithCounter++
      ) {
        let email = caseShareData.sharedWith[shareWithCounter];
        if (!(await this.isUserWithEmailListedInCase(caseCounter, email))) {
          issuesList.push(
            email + 'already shared user is not listed for in case ' + caseId
          );
        }
      }
      ...
      for (
        let shareWithCounter = 0;
        shareWithCounter < caseShareData.markedForUnShare.length;
        shareWithCounter++
      ) {
        let email = caseShareData.markedForUnShare[shareWithCounter];
        if (
          !(await this.isUserWithEmailMarkedToBeRemoved(caseCounter, email))
        ) {
          issuesList.push(
            email + 'marked to remove in not persisted ' + caseId
          );
        }
      }
    }
