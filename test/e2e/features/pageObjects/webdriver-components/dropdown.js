...
class Dropdown {
  ...
  async selectAnOption(value) {
    if (value) {
      ...
      let randomOptionArrayInt = await RandomUtils.generateRandomInt(
        1,
        await elementListSize
      );
  ...
  async _selectFromDropdownByText(dropdownOption) {
    ...
    for (const option of options) {
      ...
      if (
        optionText.trim().toLowerCase() === dropdownOption.trim().toLowerCase()
      ) {
  ...
  async _selectFromDropdownByIndex(dropdownIndex) {
    ...
    for (const option of options) {
      ...
      if (
        ...
        optionsTextArrayTemp[dropdownIndex].trim().toLowerCase()
      ) {
        optionToSelect = option;
        found = true;
        break;
  ...
  async waitForElementToBeInvisible() {
    ...
    try {
      await browser.wait(
        EC.invisibilityOf(await element(by.css(this._dropdownElement))),
        DEFAULT_TIMEOUT
      );
      return true;
    } catch (e) {
  ...
  async waitForElementToBeVisible() {
    ...
    try {
      await browser.wait(
        EC.visibilityOf($(this._dropdownElement)),
        DEFAULT_TIMEOUT
      );
      return true;
    } catch (e) {
