...
const {
  accessibilityCheckerAuditor,
} = require('../../../../accessibility/helpers/accessibilityAuditor');
...
class CaseManager {
  constructor() {
    ...
    this.caseCreateheaderLink = element(
      by.xpath(
        '//a[contains(@class ,"hmcts-primary-navigation__link")][contains(text(),"Create case")]'
      )
    );
  ...
  async startCaseCreation(jurisdiction, caseType, event) {
    ...
    while (retryOnJurisdiction < 3 && !isJurisdictionSelected) {
      try {
        ...
        cucumberReporter.AddMessage(
          'Jurisdiction option not found after 30sec. Retrying again'
        );
        retryOnJurisdiction++;
    ...
    while (startCasePageRetry < 3 && !isCaseStartPageDisplayed) {
      try {
        ...
        cucumberReporter.AddMessage(
          'Case start page not displayed in  30sec. Retrying again'
        );
        startCasePageRetry++;
      }
    }
  ...
  async startNextStep(stepName, isAccessibilityTest) {
    ...
    if (stepName) {
      nextStepSelectoption = element(
        by.xpath("//*[@id='next-step']//option[text() = '" + stepName + "']")
      );
    } else {
      nextStepSelectoption = element(
        by.xpath("//*[@id='next-step']//option[2]")
      );
    }
    ...
    switch (ccdFileTagName) {
      ...
      case 'ccd-write-yes-no-field':
        ...
        cucumberReporter.AddMessage(
          fieldName +
            ' : ' +
            (await await ccdField.$('.multiple-choice input').getText())
        );
        break;
      case 'ccd-write-fixed-list-field':
        ...
        if (!selectOption.includes(fieldName)) {
          selectOptionElement = ccdField.element(
            by.xpath("//option[contains(text() , '" + selectOption + "')]")
          );
        }
        ...
        let selectionOptionValue = await selectOptionElement.getAttribute(
          'value'
        );
      ...
      case 'ccd-write-document-field':
        ...
        await BrowserWaits.waitForCondition(async () => {
          let isUploadDone = await ccdField
            .element(by.xpath('span[contains(text(),"Uploading")]'))
            .isPresent();
      ...
      case 'ccd-write-complex-type-field':
        ...
        for (
          var fieldcounter = 0;
          fieldcounter < (await writeFields.count());
          fieldcounter++
        ) {
          var isHidden = await writeFields
            .get(fieldcounter)
            .getAttribute('hidden');
          if (isHidden) {
            continue;
          }
          var ccdSubField = writeFields
            .get(fieldcounter)
      ...
      case 'ccd-write-collection-field':
        cucumberReporter.AddMessage(
          fieldName + ' : complex write collection values'
        );
        ...
        var writeFields = ccdField.$$(
          '.panel > .form-group > .form-group>ccd-field-write'
        );
      ...
      default:
        ...
        cucumberReporter.AddMessage(
          fieldName + ' : unknown field ' + ccdFileTagName
        );
    }
  }
