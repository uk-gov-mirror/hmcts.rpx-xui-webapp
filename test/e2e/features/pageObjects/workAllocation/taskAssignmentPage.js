...
class TaskAssignmentPage extends TaskList {
  constructor() {
    ...
    this.caseWorkerSelect = $(
      'exui-task-assignment select#task_assignment_caseworker'
    );
    this.locationSelect = $(
      'exui-task-assignment select#task_assignment_location'
    );

    this.reassignBtn = element(
      by.xpath(
        '//exui-task-container-assignment//button[contains(text(),"Reassign")]'
      )
    );
    this.cancelBtn = element(
      by.xpath(
        '//exui-task-container-assignment//button[contains(text(),"Cancel")]'
      )
    );
  }
  ...
  async selectCaseworker(optionDisplayText) {
    expect(await this.amOnPage(), 'Not on Task assignment page ').to.be.true;
    await this.caseWorkerSelect
      .element(by.xpath(`//option[text() = '${optionDisplayText}']`))
      .click();
  }
