...
function loginLogoutObjects() {
  ...
  this.signOutlink = element(
    by.xpath("//a[@class='hmcts-header__navigation-link']")
  );
  this.failure_error_heading = element(
    by.css("[id='validation-error-summary-heading']")
  );
  this.dashboard_header = element(by.css("[class='govuk-heading-xl']"));

  this.incorrectCredentialsErrorHeader = element(
    by.xpath(
      '//h2[@id = "validation-error-summary-heading"][contains(text(),"Incorrect email or password")]'
    )
  );
  ...
  this.loginWithCredentials = async function (username, password) {
    ...
    CucumberReportLogger.AddMessage(
      'IDAM URL :' + (await browser.getCurrentUrl())
    );
