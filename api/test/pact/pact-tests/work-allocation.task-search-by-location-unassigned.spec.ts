    ...
    for (const combination of LOCATION_COMBINATIONS) {
      ...
      for (const order of ['ascending', 'descending']) {
        ...
        const tasks = sortTasks(
          filterByLocations(baseTasks, values),
          key,
          order
        );

        describe(`when requested to search for unassigned tasks at any of [${values.join(
          ', '
        )}] in ${order} order of ${key}`, () => {
          before(() =>
            provider.addInteraction({
              state: 'a list of appropriate tasks are returned',
              uponReceiving: `a valid request to search for unassigned tasks at any of [${values.join(
                ', '
              )}] in ${order} order of ${key}`,
              withRequest: {
                method: 'POST',
          ...
          it('returns success with a 200', async () => {
            ...
            const { status } = await handleTaskSearch(
              taskUrl,
              request,
              {} as EnhancedRequest
            );
