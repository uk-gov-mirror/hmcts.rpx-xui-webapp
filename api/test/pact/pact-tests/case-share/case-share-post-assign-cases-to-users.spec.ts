...
describe('Post Cases from CaseAssignment Api', () => {
  ...
  const mockResponse: AssignAccessWithinOrganisationDto = {
    status_message: somethingLike(
      'Role [Defendant] from the organisation policy successfully assigned to the assignee.'
    ),
  };
  ...
  describe('When Cases are assigned to Users', () => {
    ...
    it('Returns CaseAssingments Response', async () => {
      ...
      resp.then((axResponse) => {
        ...
        const responseDto: AssignAccessWithinOrganisationDto = axResponse.data as AssignAccessWithinOrganisationDto;
        try {
...
function assertCaseAssignmentResponses(
  response: AssignAccessWithinOrganisationDto
) {
