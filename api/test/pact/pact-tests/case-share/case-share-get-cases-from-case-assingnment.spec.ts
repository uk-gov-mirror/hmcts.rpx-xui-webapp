  ...
  describe('when requested to get case assignments for array of CaseIds ', () => {
    ...
    it('Returns CaseAssingments Response', async () => {
      ...
      resp.then((axResponse) => {
        const responseDto: CaseAssignmentResponseDto = axResponse.data as CaseAssignmentResponseDto;
        try {
          assertCaseAssignmentResponses(responseDto);
        } catch (e) {
          e.toString(
            `~~~~~Error when trying to assert the response from the call to the ${taskUrl}` +
              e
          );
        }
      });
...
const caseAssignmentResponseDto: CaseAssignmentResponseDto = {
  status_message: somethingLike(
    'Case-User-Role assignments returned successfully'
  ),
...
function assertCaseAssignmentResponses(response: CaseAssignmentResponseDto) {
  ...
  expect(response.case_assignments[0].sharedWith[0].first_name).to.be.equal(
    'Bill'
  );
  expect(response.case_assignments[0].sharedWith[0].last_name).to.be.equal(
    'Roberts'
  );
  expect(response.case_assignments[0].sharedWith[0].case_roles[0]).to.be.equal(
    'Claimant'
  );
  expect(response.case_assignments[0].sharedWith[0].case_roles[1]).to.be.equal(
    'Defendant'
  );
}
