...
describe('Work Allocation API', () => {
  ...
  describe('when requested to complete a task', () => {
    ...
    it('returns success with a 200', async () => {
      ...
      const { status } = await handleTaskPost(
        taskUrl,
        BEHAVIOURS.SUCCESS,
        {} as EnhancedRequest
      );
  ...
  describe('when making a bad request to complete a task', () => {
    ...
    it('returns failure with a 400', async () => {
      ...
      try {
        response = await handleTaskPost(
          taskUrl,
          BEHAVIOURS.BAD_REQUEST,
          {} as EnhancedRequest
        );
      } catch (err) {
        response = err;
