...
export function successInterceptor(response) {
  ...
  logger.info(
    `Success on ${response.config.method.toUpperCase()} to ${url} (${
      response.duration
    })`
  );
  return response;
...
export function errorInterceptor(error) {
  ...
  if (!data) {
    data = valueOrNull(error, 'response.status')
      ? JSON.stringify(error.response.data, null, 2)
      : null;
    if (!data) {
      data = error;
    }
    logger.error(`Error on ${error.config.method.toUpperCase()} to ${url} in (${
      error.duration
    }) - ${error} \n
