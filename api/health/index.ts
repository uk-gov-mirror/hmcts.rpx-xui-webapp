...
const config = {
  checks: {
    ccdComponentApi: checkServiceHealth(
      getConfigValue(SERVICES_CCD_COMPONENT_API_PATH)
    ),
    ccdDataApi: checkServiceHealth(
      getConfigValue(SERVICES_CCD_DATA_STORE_API_PATH)
    ),
    documentsApi: checkServiceHealth(
      getConfigValue(SERVICES_DOCUMENTS_API_PATH)
    ),
...
export const addReformHealthCheck = (app) => {
  if (showFeature(FEATURE_TERMS_AND_CONDITIONS_ENABLED)) {
    config.checks = {
      ...config.checks,
      ...{
        termsAndConditions: checkServiceHealth(
