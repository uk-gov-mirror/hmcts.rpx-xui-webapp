...
export async function getUsers(
  ...
  try {
    ...
    const { status, data }: { status: number; data: any } = await handleGet(
      path,
      req,
      next
    );
  ...
  try {
    ...
    for (const caseUser of caseUsers) {
      const sharedCase: SharedCase = {
        ...
        sharedWith: caseUser.shared_with.map((rawSharedWith) =>
          ccdToUserDetails(rawSharedWith)
        ),
      };
...
export async function assignCases(
  ...
  } = await doShareCase(req, shareCases);

  // TODO: call unshare case api
  // await doUnshareCase(req, shareCases, updatedSharedCases)

  const originalSharedNumber = shareCases.reduce(
    (acc, aCase) =>
      acc + (aCase.pendingShares ? aCase.pendingShares.length : 0),
    0
  );
  const afterSharedNumber = updatedSharedCases.reduce(
    (acc, aCase) =>
      acc + (aCase.pendingShares ? aCase.pendingShares.length : 0),
    0
  );
  ...
  results.forEach((result) => {
    ...
    if (status === 'rejected') {
      ...
      updatedErrorMessages.push(
        `{request: ${reason.config.data}, response: {${reason.data.status} ${reason.data.message}}}`
      );
    }
  });
  const updatedSharedCases: SharedCase[] = handleRejectedPayloads(
    shareCases,
    rejectedPayloads
  );
  return {
...
function promiseCaseShareBatchCall(
  shareCases: SharedCase[],
  path: string,
  req: EnhancedRequest
) {
  ...
  for (const aCase of shareCases) {
    ...
    for (let i = 0; i < newPendingShares.length; i++) {
      const foundFailed = rejectedPayloads
        ...
        .some(
          (foundPayload) =>
            foundPayload.assignee_id === newPendingShares[i].idamId
        );
